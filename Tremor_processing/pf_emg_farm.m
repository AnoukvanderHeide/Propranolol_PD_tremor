...
for a = 1:nSub
    ...
    for b = 1:nSess
        ...
        for c = 1:nRun
            ...
            tempName = strrep(conf.file.name, 'CurSub/&/CurSess', [CurSub, '_', CurSess, '_'])
            
            %CurFile    =   pf_findfile([conf.dir.root 'sub-' CurSub '/Brainvision'],tempName,'conf',conf,'CurSub',a,'CurSess', b, 'CurRun',c);
            CurFile = pf_findfile(conf.dir.emgFixed, tempName, 'conf',conf,'CurSub',a,'CurSess', b, 'CurRun',c);
            ...
            %=============================FILES===============================%
            %Files{cnt,1}.raw  =  fullfile([conf.dir.root 'sub-' CurSub '/Brainvision'],CurFile);
            Files{cnt,1}.raw  =  fullfile([conf.dir.emgFixed],CurFile);
...
for a = 1:nFiles
    ...
    if ~sum(abs(([ve(2:end).latency]-[ve(1:end-1).latency])/EEG.srate-conf.file.scanpar(1))>0.1)
        ...
        elseif length(EEGalt.data) > length(EEG.data)
            ...
            %%MW: Hier gaat t mis dat als we 1:4 7:8 meten
            %%EEG.data     = vertcat(EEG.data,EEGalt.data);
            
            %%MW: zo zet ie ze (als t goed is) weer op dezelfde indices terug 
            %%als waar ze vandaan kwamen ipv dat ie ze achter elkaar plakt
            newData(conf.file.chan,:) = EEG.data;       %EEG.data indices:      conf.file.chan
            newData(iAlt,:) = EEGalt.data;              %EEGalt.data indices:   iAlt
            EEG.data = newData;
        ...
        newChanlocs(conf.file.chan) = EEG.chanlocs;
        newChanlocs(iAlt) = EEGalt.chanlocs;
