...
for a = 1:nSub
    ...
    for b = 1:nSess
        ...
        for c = 1:nRun
            ...
            if ~isfield(conf.mkregr,'data') % If the gui was not used
                %%MW: zodat ie de goede files pakt
                tempfile = strrep(conf.mkregr.file, 'CurSub/&/CurSess', [CurSub, '_', CurSess, '_']);
                file   =   pf_findfile(conf.dir.prepemg,tempfile,'conf',conf,'CurSub',a,'CurSess',b,'CurRun',c,'msgM',0);
            else
                file   =   'GUI_dataset';
            end
            ...
            if strcmp(conf.mkregr.nscan,'detect')
                for d = 1:length(conf.dir.fmri.preproc)
                    ...
                    if d == 1
                        %%MW: hier kan t mogelijk fout gaan (detect heeft
                        %%tot nu toe nooit op true gestaan dus nooit
                        %%getest)
    ...
    if ~isfield(conf.mkregr,'data') % If the gui was not used
        ...
        if strcmp(conf.mkregr.reanalyze,'yes')
            if strcmp(conf.mkregr.reanalyzemeth,'regressor')
                %%MW: hier kan t mogelijk fout gaan (reanalyze heeft tot nu
                %%toe altijd op 'no' gestaan en heb het nog niet getest)
            ...
            elseif strcmp(conf.mkregr.reanalyzemeth,'ps_save')
                %%MW: hier kan t mogelijk fout gaan 
        ...
        elseif strcmp(conf.mkregr.automatic,'yes')    
            ...
            tosearch.powspctrm = dat.powspctrm(conf.mkregr.automaticchans,...
                find(dat.freq>conf.mkregr.automaticfreqwin(1) & dat.freq<conf.mkregr.automaticfreqwin(2)),:);
...
%               plot(tosearch.freq,nanmean(squeeze(tosearch.powspctrm(i,:,:)),2))
                plot(dat.freq,nanmean(squeeze(dat.powspctrm(conf.mkregr.automaticchans(i),:,:)),2))
            end
            plot(tosearch.freq(c),max(mean_powspctrm(:)),'or','LineWidth',5 )
            %Plot lines from frequency window
            xline(conf.mkregr.automaticfreqwin(1), ':')
            xline(conf.mkregr.automaticfreqwin(2), ':')
            hold off
...
%             saveas(gcf,fullfile(conf.mkregr.automaticdir,savename),'fig');
            %%MW: so it saves the figure as mat figure too
    ...
    for i = 1:nFreq
        ...
        if strcmp(conf.mkregr.plotcond,'yes')       
            ...
            %[onset,offset,~]    =   pf_fmri_creacond(evefile,dat.startscan_sca-1,conf.mkregr.mrk,dat.fs,dat.mtr); %%MW: bestaat niet
            
            onset = [6:60*2:60*9];
            offset = [65:60*2:60*10];
        ...
        for  b = 1:nMeth
            ...
            for c = 1:size(R,2)/2
                ...
                if strcmp(conf.mkregr.plotcond,'yes')  %%MW: ook voor plotten
                    Y   =   get(gca,'ylim');
