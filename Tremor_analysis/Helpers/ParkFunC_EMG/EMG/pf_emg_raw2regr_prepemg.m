...
for a = 1:nSub
    ...
    for b = 1:nSess
        ...
        %%MW: zodat ie de goede files pakt (gaat fout bij 011_01 bijv waar ie ook 011_02 pakt)
        datstring = strrep(conf.prepemg.datfile, 'CurSub/&/CurSess', [CurSub, '_', CurSess, '_']);
        mrkstring = strrep(conf.prepemg.mrkfile, 'CurSub/&/CurSess', [CurSub, '_', CurSess, '_']);
        hdrstring = strrep(conf.prepemg.hdrfile, 'CurSub/&/CurSess', [CurSub, '_', CurSess, '_']);
        CurDat  =   pf_findfile(conf.dir.preproc,datstring,'conf',conf,'CurSub',a,'CurSess',b,'CurRun',c);
        CurMrk  =   pf_findfile(conf.dir.preproc,mrkstring,'conf',conf,'CurSub',a,'CurSess',b,'CurRun',c);
        CurHdr  =   pf_findfile(conf.dir.preproc,hdrstring,'conf',conf,'CurSub',a,'CurSess',b,'CurRun',c);
...
for a = 1:nFiles
    ...
    for b = 1:nPlot
        ...
        if ~ischar(CurPlot)
            ...
            legend(h,CurFreq.label,'interpreter','none')    %%MW: hier miss de namen aanpassen?
            legend('boxoff')
    ...
    saveas(gcf,fullfile(conf.dir.prepemg,[CurSub '_' CurSess '_' CurRun '_powerspectrum.fig']),'fig');
