...
ruwDir=[regexprep(pwd, '(^.*)(Onderzoek.*)', '$1') 'Onderzoek/fMRI/' study '/ruw/' pp '/log/'];
...
while ~feof(fid)
    ...
    if ~numel(regexp(line,'^#'))&&numel(line)>0 % allow for comments in filex.txt file.
        ...
        if ~strcmp(parts{6},'xx')
            ...
            destDir=[regexprep(pwd, '(^.*)(Onderzoek.*)', '$1') 'Onderzoek/fMRI/' study '/pp/' pp '/' task '/regressor/'];
            ...
            if numel(regexp(filename,'motor'))>0
            ...
            disp(filename)
            if numel(regexp(filename,'tremor'))>0
            ...
            if numel(regexp(filename,'gonogo'))>0
