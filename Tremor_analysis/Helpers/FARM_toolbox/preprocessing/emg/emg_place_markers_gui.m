...
function [bursts thr]=emg_calculate_mode_gui(data,vec,params)
    ...
    for ch=params.doMuscles
        ...
        for i=1:numel(vec)
            ...
            for j=1:size(vec{i},1)
                ...
                mode=my_mode_vector(s(p),2,2048,round(0.4*2048));
            ...
            m=emg_marker_routine(sdhf,threshhigh*mv,threshlow*mv);
            
            for k=1:numel(m.b)
                
                bursts{ch}(end+1).b=m.b(k);
                bursts{ch}(end).e=m.e(k);
