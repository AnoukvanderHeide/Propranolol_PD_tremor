...
% make sure that there are enough points in the mode estimator!
% secs=6;
% srate=1024;
% speedup=50;
%
...
function out=my_mode_vector_fast(v,secs,srate,speedup)
try
    ...
    if ~sum(isnan(v))
        ...
        % keyboard;
        points=(wsize/2+1):speedup:(numel(v)-wsize/2);
        ...
        modevec=zeros(numel(v),1);
        sumvec=zeros(numel(v),1);
        
        for i=1:numel(points);
            
            ind=points(i)-wsize/2:(points(i)+wsize/2-1);
            ...
            range=find(count>0.00*mean(sc(1:3)),1,'first'):find(count>0.8*mean(sc(1:3)),1,'last');
            ...
            % search the minimum.
            % keyboard;
            P=fminsearch(@(p) my_quadratic_function(p,[0 y],ps),ps./ps,options);
            P=P.*ps;
            ...
            % now check the mode... if it is a nonesensical number, do not take
            % it with us. Otherwise, 'add' the mode value to the modevec, and
            % add 1 to the sumvec.
            
            % als modus < 0 of als modus > 5* de median, dan is ie onzin.
            if mode>0&&mode<5*mt
                % add it!
                modevec(ind)=modevec(ind)+mode;
                sumvec(ind)=sumvec(ind)+1;
            else
                disp(sprintf(['bad mode-estimate@ points; ' num2str(ind(1)) ' - ' num2str(ind(end)) ', val = ' num2str(mode) '\n']));
            end
        ...
        % only calculate for mode > zero!
        % keyboard;
        mvec=zeros(size(v));
        mvec(sumvec>0)=modevec(sumvec>0)./sumvec(sumvec>0);
        
        % a 2-sec moving average window filter applied...
        mvec=filtfilt(ones(1,srate*2)/srate/2,1,mvec);
        
        out=reshape(mvec,size(v));
    ...
    out=zeros(size(v));
    disp('it didnt work!! --> returning zeros to keep going');
end
