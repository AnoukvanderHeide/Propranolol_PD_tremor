...
%MW: because of the shift, the last datapoints (nr = phaseShiftSamples) are 
%removed, so it's possible that one of the triggermarkers is now after the 
%last data point (if there was too little data after the scan markers and 
%endRunWin had to be made smaller a bit back in the code), so remove those 
%scan markers now


function brainampconverter(eegFilename, outputFolder)
...
% outputFolder = fullfile(conf.dir.output, ['sub-' conf.sub.curSub]);
...
if sum(nrCinds>2)==1 %only one bigger than 2
    ...
    rCinds = find(strcmp(cell_evt(:,1),ResponseCode) & ~strcmp(cell_evt(:,2), 'inverse')); %Get indices of response codes
...
for i=(1-FullFileConversion):numel(runTriggers)
    ...
    if numel(HRchannel)>0
        ...
        % MW: because of the shift, the last datapoints (nr =
        % phaseShiftSamples) are removed, so it's possible that one of the
        % triggermarkers is now after the last data point (if there was too
        % little data after the scan markers and endRunWin had to be made
        % smaller a bit back in the code), so remove those scan markers now
        newdstriggerSamps = dstriggerSamps(find(dstriggerSamps <= length(ts_hpf_lpf_dspulsedat)));
        if length(newdstriggerSamps) < length(dstriggerSamps) | ~all(newdstriggerSamps == dstriggerSamps)
            warning('removed %i scan marker(s)', length(dstriggerSamps) - length(newdstriggerSamps))
        end
        dstriggerSamps = newdstriggerSamps;
        
            
        %Check if converted puls file already exists
%         [pulspath, pulsfile]=fileparts(eegFilename);
        [~,pulsfile]=fileparts(eegFilename);
        pulspath = outputFolder;
...
%         [autpath,autfile]=fileparts(eegFilename);
        [~,autfile]=fileparts(eegFilename);
        autpath = outputFolder;
